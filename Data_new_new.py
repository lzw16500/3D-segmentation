...
if __name__ == "__main__":
    ...
    if not os.path.exists(folder_out):
        os.mkdir(folder_out)
    dataset_path = os.path.join(os.getcwd(), "data_raw_original_ply")

    datasets_path = [os.path.join(dataset_path, file_name) for file_name in os.listdir(dataset_path)]
    count = 0
    for pp in datasets_path:
        file_name = os.getcwd() + f"\\data_raw_original_ply\\downsample{count}.ply"
        # pp = o3d.io.read_point_cloud(pp)
        mesh = o3d.io.read_triangle_mesh(f"pp{count}_original_labeled.ply")
        # pp = o3d.geometry.PointCloud.uniform_down_sample(pp, 5)
        mesh_smp = mesh.simplify_quadric_decimation(target_number_of_triangles=40000)
        o3d.io.write_triangle_mesh(file_name, mesh_smp)
        # o3d.io.write_point_cloud(file_name, pp)

        mesh_tm = tm.load_mesh(file_name)
        ...
        # save the dictionary
        _, full_file_name = os.path.split(file_name)
