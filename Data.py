...
def pick_points(pcd):
    print("")
    print(
        "1) Please pick 2 points using [shift + left click]"
    )
...
def calculate_angle_matrix(Face_normals_list,neighbor_index_list):
    ...
    for i in range(Number_Face):
        ...
        angle_1 = calculate_angle(neighbor_1_normals,neighbor_2_normals)
        angle_2 = calculate_angle(neighbor_1_normals, neighbor_3_normals)
        angle_3 = calculate_angle(neighbor_2_normals, neighbor_3_normals)
...
def aligment(pcd ):
    ...
    coordinations1 = points[picked_points[0]][:]
    coordinations2 = points[picked_points[1]][:]
    center_coordnation = (coordinations1 + coordinations2) / 2
    vector = coordinations1 - coordinations2
    axis_x, axis_y, axis_z = [1, 0, 0], [0, 1, 0], [0, 0, 1]
    #translate
    o3d.geometry.PointCloud.translate(mesh, -center_coordnation)
# rotat around z-axis
    project_xy = vector - [0, 0, vector[2]]
    angle_z = calculate_angle(project_xy, axis_y)
    rot_mat_z = rotat_mat(axis_z, angle_z)
    vector = np.dot(rot_mat_z, vector)
    o3d.geometry.PointCloud.rotate(mesh, rot_mat_z, center=(0, 0, 0))
#rotat around x-axis
    project_zy = vector - [vector[0], 0, 0]
    angle_x = calculate_angle(project_zy, axis_y)
    rot_mat_x = rotat_mat(axis_x, -angle_x)
    vector = np.dot(rot_mat_x, vector)
    o3d.geometry.PointCloud.rotate(mesh, rot_mat_x,center=(0,0,0))
#reflection
    rot_mat = rotat_mat(axis_z,3.1415926533)
    o3d.geometry.PointCloud.rotate(mesh, rot_mat, center=(0,0,0))

def rotat_mat(axis,radian):
...
if __name__ == "__main__":
    mesh = o3d.io.read_triangle_mesh('pp1_labeled.ply')
    mesh_tm = tm.load_mesh('pp1_labeled.ply')
